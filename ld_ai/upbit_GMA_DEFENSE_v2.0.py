...
def get_cash():
    ...
    for i in range(m):
        while True:
            try:
                value = upbit.get_balance(assets[i]) * pyupbit.get_current_price(assets[i])
                s += value
                break
...
while True:
    ...
    price_data = np.concatenate([dfs[i]['close'].values.reshape((-1,1)) for i in range(m)], axis=1)
            ...
            for i in range(m):
                while True:
                    try:
                        ...
                        text = "Error occured when selling {}... waiting for the issue to be resolved".format(assets[i])
                        telegram_bot.sendMessage(chat_id=chat_id, text=text)
                        print(e)
                        time.sleep(3)
                time.sleep(1)
    ...
    if iterations == 0:
        ...
        for i in range(m):
            while True:
                try:
                    upbit.buy_market_order(assets[i], cash_amount * (1-gamma/100) * defensive_bah_portfolio[i])
                    break
                except Exception as e:
                    text = "Error occured when buying {}... waiting for the issue to be resolved".format(assets[i])
                    telegram_bot.sendMessage(chat_id=chat_id, text=text)
                    print(e)
                    time.sleep(3)
            time.sleep(1)
        ...
        if best_profits > 1 + threshold:
            ...
            if choice == 1:
                ...
                for i in range(m):
                    while True:
                        try:
                            upbit.buy_market_order(assets[i], cash_amount * (1-gamma/100) * best_portfolio[i])
                            break
                        except Exception as e:
                            text = "Error occured when buying {}... waiting for the issue to be resolved".format(assets[i])
                            telegram_bot.sendMessage(chat_id=chat_id, text=text)
